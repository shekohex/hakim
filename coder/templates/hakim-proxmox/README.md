...
5. Template runs the same module stack used by Docker template (`shekohex-agent`, `openclaw-node`, `code-server`, etc).
...
Install the default hook script on the Proxmox host when using persistent bind mounts:
...
- On `pre-start`, detect the `/home/coder` mount source from `pct config <vmid>`.
- If the source is an absolute host path, ensure it exists and is writable (for example `install -d -m 0777 <path>`).
